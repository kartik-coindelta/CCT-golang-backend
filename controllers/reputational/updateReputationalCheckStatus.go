...
// UpdateReputationalCheckStatus updates the status of an reputational check by ID
func UpdateReputationalCheckStatus(c *gin.Context) {
	...
	updateFields := bson.M{"reputational.status": updateData.Status}

	// Get the reputationalCheck collection from the database
	collection := db.GetCollection("reputationalchecks")

	// Call UpdateItem to perform the update
	result, err := db.UpdateItem(reputationalID, updateFields, collection)
	if err != nil {
