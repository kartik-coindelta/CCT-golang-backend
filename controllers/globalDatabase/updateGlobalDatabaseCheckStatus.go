...
// UpdateGlobalDatabaseCheckStatus updates the status of an globalDatabase check by ID
func UpdateGlobalDatabaseCheckStatus(c *gin.Context) {
	...
	updateFields := bson.M{"globalDatabase.status": updateData.Status}

	// Get the globalDatabaseCheck collection from the database
	collection := db.GetCollection("globaldatabasechecks")

	// Call UpdateItem to perform the update
	result, err := db.UpdateItem(globalDatabaseID, updateFields, collection)
	if err != nil {
