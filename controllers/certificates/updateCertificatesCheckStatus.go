...
// UpdateCertificatesCheckStatus updates the status of an certificates check by ID
func UpdateCertificatesCheckStatus(c *gin.Context) {
	...
	updateFields := bson.M{"certificates.status": updateData.Status}

	// Get the certificatesCheck collection from the database
	collection := db.GetCollection("certificateschecks")

	// Call UpdateItem to perform the update
	result, err := db.UpdateItem(certificatesID, updateFields, collection)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to update the Certificates."})
		return
	}
	...
	if err := result.Decode(&updatedCertificatesCheck); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to decode updated Certificates."})
		return
	}
