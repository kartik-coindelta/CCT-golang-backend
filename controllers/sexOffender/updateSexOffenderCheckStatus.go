...
// UpdateSexOffenderCheckStatus updates the status of an sexOffender check by ID
func UpdateSexOffenderCheckStatus(c *gin.Context) {
	...
	updateFields := bson.M{"sexOffender.status": updateData.Status}

	// Get the sexOffenderCheck collection from the database
	collection := db.GetCollection("sexoffenderchecks")

	// Call UpdateItem to perform the update
	result, err := db.UpdateItem(sexOffenderID, updateFields, collection)
	if err != nil {
