...
// UpdateAddressCheckStatus updates the status of an professionalLicense check by ID
func UpdateAddressCheckStatus(c *gin.Context) {
	...
	updateFields := bson.M{"professionalLicense.status": updateData.Status}

	// Get the professionalLicenseCheck collection from the database
	collection := db.GetCollection("professionallicensechecks")

	// Call UpdateItem to perform the update
	result, err := db.UpdateItem(professionalLicenseID, updateFields, collection)
	if err != nil {
