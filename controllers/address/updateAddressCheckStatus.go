...
// UpdateAddressCheckStatus updates the status of an address check by ID
func UpdateAddressCheckStatus(c *gin.Context) {
	...
	updateFields := bson.M{"address.status": updateData.Status}

	// Get the addressCheck collection from the database
	collection := db.GetCollection("addresschecks")

	// Call UpdateItem to perform the update
	result, err := db.UpdateItem(addressID, updateFields, collection)
	if err != nil {
