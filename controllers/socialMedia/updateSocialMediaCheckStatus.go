...
// UpdateSocialMediaCheckStatus updates the status of an socialMedia check by ID
func UpdateSocialMediaCheckStatus(c *gin.Context) {
	...
	updateFields := bson.M{"socialMedia.status": updateData.Status}

	// Get the socialMediaCheck collection from the database
	collection := db.GetCollection("socialMediachecks")

	// Call UpdateItem to perform the update
	result, err := db.UpdateItem(socialMediaID, updateFields, collection)
	if err != nil {
