...
// UpdateExperienceCheckStatus updates the status of an experience check by ID
func UpdateExperienceCheckStatus(c *gin.Context) {
	...
	updateFields := bson.M{"experience.status": updateData.Status}

	// Get the experienceCheck collection from the database
	collection := db.GetCollection("experiencechecks")

	// Call UpdateItem to perform the update
	result, err := db.UpdateItem(experienceID, updateFields, collection)
	if err != nil {
