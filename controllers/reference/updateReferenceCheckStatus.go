...
// UpdateReferenceCheckStatus updates the status of an reference check by ID
func UpdateReferenceCheckStatus(c *gin.Context) {
	...
	updateFields := bson.M{"reference.status": updateData.Status}

	// Get the referenceCheck collection from the database
	collection := db.GetCollection("referencechecks")

	// Call UpdateItem to perform the update
	result, err := db.UpdateItem(referenceID, updateFields, collection)
	if err != nil {
