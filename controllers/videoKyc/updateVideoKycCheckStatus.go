...
// UpdateVideoKycCheckStatus updates the status of an videoKyc check by ID
func UpdateVideoKycCheckStatus(c *gin.Context) {
	...
	updateFields := bson.M{"videoKyc.status": updateData.Status}

	// Get the videoKycCheck collection from the database
	collection := db.GetCollection("videokycchecks")

	// Call UpdateItem to perform the update
	result, err := db.UpdateItem(videoKycID, updateFields, collection)
	if err != nil {
